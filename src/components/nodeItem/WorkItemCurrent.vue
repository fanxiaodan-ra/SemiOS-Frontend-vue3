<template>
  ...
  <template v-else-if="props.dataObj.daoStatus === 3">
    <div class="h-[114px] text-grey-1 text-xs text-center leading-[114px]">
      This DAO is no longer in progress, so the mint window information is not
      available.
    </div>
  </template>
  <template v-else>
    ...
    <div>
      <v-row align="start" no-gutters class="current-num">
        ...
        <v-col class="my-flexaj" cols="4" v-if="dataObj?.workStatus === 0">
          {{ bigNumFormat(info?.mintersMaxRewardToken, 5, 0.000001) }}
          <TokenIcon
            mgl="6px"
            :daoToken="true"
            :daoSymbol="dataObj.daoSymbol"
          ...
          <span v-if="!props.topupMode" class="my-flexaj">
            +
            {{ bigNumFormat(info?.mintersMaxRewardEth, 5, 0.000001) }}
            <TokenIcon
              mgl="6px"
              :payCurrencyType="dataObj.payCurrencyType"
              :inputTokenAddress="dataObj.inputTokenAddress"
      ...
      <v-row align="start" no-gutters class="current-name">
        ...
        <v-col cols="4" v-if="dataObj?.workStatus === 0"
...
const init = async () => {
  try {
    loading.value = true
    const workId = props.dataObj.workId
    const data = await workStore.getWorkCurBlockWinInfo({
      workId,
    })
    info.value = data
  } catch (error) {
