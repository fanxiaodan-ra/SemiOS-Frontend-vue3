<template>
  <v-card class="node-card">
    <div class="maker-title">
      <h3>
        Top-up Balance : ERC-20 Tokens Unlock ETH
        <i class="iconfont icon-info ft18">
          <v-tooltip activator="parent" location="top" max-width="300">
            Proportion of ETH unlocked when Using Top-up to unlock ETH
          </v-tooltip>
        </i>
      </h3>
    </div>
    ...
    <div class="card-body" v-else>
            ...
            >
              <div>
                <v-btn class="text-none" variant="text">
                  ...
                  {{ truncateString(item.daoName, 12) }}
                </v-btn>
              </div>
            </router-link>
          </v-col>
      ...
      <div>
        <v-btn block class="btnz text-none" type="submit" @click="submitFormEth"
          >Submit</v-btn
        >
      </div>
    </div>
  ...
  <DialogLoading
    ...
    text="Your modification is being processed, it should be confirmed on the blockchain shortly."
  />
</template>
...
import { setTopUpInputSplitRatio } from '@/common/web3service'
...
const submitFormEth = async () => {
  ...
  if (
    ...
    ownerAddress.value.toLowerCase()
  )
    return notifyErr('You don’t have permission to edit this parameter.', true)
  isDialogLoading.value = true
  try {
    const defaulData = formData.value.tokenTransEthList[0]
    const subDaoIds = formData.value.tokenTransEthList
      .filter((item: any) => item.daoId !== null)
      .map((item: any) => item.projectId)
    const erc20Ratios = formData.value.tokenTransEthList
      .filter((item: any) => item.daoId !== null)
      .map((item: any) =>
...
import { oninputNum, minusOthers, truncateString } from '@/utils'
...
const getData = async () => {
  try {
    ...
    formData.value.tokenTransEthList = [
      {
