<template>
  <v-card class="node-card">
    <div class="maker-title">
      <h3>
        {{ t('daoCollectionAnalytics.rightMakerTopupBalanceERC20Tokens.title') }}
        <i class="iconfont icon-info ft18">
          <v-tooltip activator="parent" location="top" max-width="300">
            {{ t('daoCollectionAnalytics.rightMakerTopupBalanceERC20Tokens.desc') }}
          </v-tooltip>
        </i>
      </h3>
    </div>
    ...
    <div class="card-body" v-else>
      <div class="maker-box">
        <v-row no-gutters class="rowlin" v-for="(item, idx) in formData.tokenTransEthList" :key="item.daoName">
          ...
          <v-col cols="4" class="e6name">
            <v-text-field label="Wallet" density="comfortable" append-inner-icon="mdi-percent-outline"
              type="number"
              v-model="item.walletRatio" @update:modelValue="
