<template>
  <v-card class="node-card">
    <div class="card-title">
      <h3>SubNodes Asset Pool ERC-20 Tokens</h3>
    </div>
...
import { analyticsAssetPoolToken } from '@/api/daos'
...
const getData = async () => {
  const query = { daoId: route.query.id }
  const res = (await analyticsAssetPoolToken(query)) as any
  chartData.value = {
    xdata: res.data.time,
